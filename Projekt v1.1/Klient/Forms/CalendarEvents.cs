...
namespace Klient.Forms
{
    public partial class CalendarEvents : Form
    {
        ...
        //private string adress = "192.168.1.15"; //ubuntu nowe
        ...
        private void ConnectCallbackText(IAsyncResult ar)
        {
            try
            {
                ...
                while (buffer == Encoding.ASCII.GetBytes("got"))
                {
                    socketFd.Receive(buffer);
                }
                string currentLine = "";
                byte[] curLine = Encoding.ASCII.GetBytes("");
                //WYSYŁANIE LINIA PO LINI ZAWARTOŚCI KALENDARZA
                for (int i = 0; i < calendarsStruct.calendarDays.Count; i++)
                {
                    currentLine += (calendarsStruct.calendarDays[i].day + "." + calendarsStruct.calendarDays[i].month + "." +
                ...
                //currentLine += "\n\n";
                curLine = Encoding.ASCII.GetBytes(currentLine);
                ...
                //if(Int32.Parse(buffer.ToString()) == 0)
               // {
